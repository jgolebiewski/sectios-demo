...
import { CustomDropdown } from '@reports/ui';
...
export const RouteComponent: FC<RouteComponentProps> = ({ index }): JSX.Element => {
    ...
    return (
        <RouteWrapper>
            <h4>Route</h4>
            <CustomTextField name={`routes.${index}.name`} control={control} label="Name" />
            <CustomDatePicker name={`routes.${index}.startDate`} control={control} label="Start date" />
            <CustomDatePicker name={`routes.${index}.endDate`} control={control} label="End date" />
            <CustomTextField name={`routes.${index}.duration`} control={control} label="Route duration" disabled />
            <CustomDropdown name={`routes.${index}.type`} control={control} label="Route type" options={routeTypes} />
        </RouteWrapper>
    );
};
